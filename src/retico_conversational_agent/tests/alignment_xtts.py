...
def get_words_durations(alignment_required_data):
    ...
    for i in range(len(att_attentions)):
        words_durations_in_nb_frames, words_durations_in_sec, alignments = get_dur(
            gpt_cond_latent_shape[i], text_tokens[i], att_attentions[i], detokenized_text_tokens[i]
        )
